...
```bash
MOUNT_NAME="$(awk -v root="$HOME/nas_docs/" '$0 !~ /^[[:space:]]*#/ && $3 == "cifs" && index($2, root) == 1 { sub(root, "", $2); print $2; exit }' /etc/fstab)"
test -n "$MOUNT_NAME"
...
OpenClaw 컨테이너는 NAS mount를 /home/node/nas_docs/host-<hash>/SHARE에서 읽음
Hermes 컨테이너는 NAS mount를 /workspace/nas_docs/host-<hash>/SHARE에서 읽음
