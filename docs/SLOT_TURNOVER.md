...
```bash
sudo /opt/openclaw-nas-agent-baseline/scripts/svcops-control.sh nas-register "$TARGET_USER" '//NAS_HOST/SHARE'
```
...
agent-nas-mount --request-share "$NAS_SHARE" --reset-credential --wait
agent-nas-mount --share "$NAS_SHARE" --status
```
...
OpenClaw 컨테이너는 NAS mount를 /home/node/nas_docs/host-<hosthash>/SHARE-<sharehash>에서 읽음
Hermes 컨테이너는 NAS mount를 /workspace/nas_docs/host-<hosthash>/SHARE-<sharehash>에서 읽음
